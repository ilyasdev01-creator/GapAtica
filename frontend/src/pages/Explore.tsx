        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {
            domains.map((domain, index) => {
              ...
              return (
                <div key={index} className={`relative group ${domain.comingSoon ? 'opacity-60' : ''}`}>
                  ...
                  {domain.comingSoon && (
                    <div className="absolute inset-0 backdrop-blur-md bg-black/30 rounded-xl z-10 flex items-center justify-center pointer-events-none">
                      <span className="bg-[#00D0A6] text-black px-4 py-2 rounded-full font-semibold text-sm">Coming Soon</span>
                    </div>
                  )}
                  ...
                  <button
                    ...
                    onClick={() => !domain.comingSoon && console.log(`Exploring ${domain.name}`)}
                  ...
                  >
                    {/* Icon */}
                    <div className={`text-5xl mb-4 transition-transform duration-300 ${domain.comingSoon ? '' : 'group-hover:scale-110'}`}>
                      <DomainIcon className={domain.comingSoon ? 'text-[#6b7280]' : 'text-[#00D0A6] drop-shadow-lg'} />
                    </div>

                    {/* Content */}
                    <h2 className={`text-2xl font-bold mb-2 ${domain.comingSoon ? 'text-gray-400' : 'text-white'}`}>
                      {domain.name}
                    </h2>
                    <p className={`text-sm leading-relaxed ${domain.comingSoon ? 'text-gray-500' : 'text-[#9FD6FF]'}`}>
                      {domain.description}
                    </p>
